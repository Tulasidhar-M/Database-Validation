...
def compare_objects(common_in_src_and_dest, src_db, tgt_db, result_df, user_email, exception_df):
    ...
    for i in common_in_src_and_dest:
        ...
        query = f"""select (select count(*) from {src_db}.{i}) - (select count(*) from {tgt_db}.{i}) as CNT"""
        ...
        try:
            ...
            if val!=0:
                new_row = {'object_name':i, 'mismatch_rows_count':val,'query':query}
                new_df = pd.DataFrame([new_row])
                result_df = pd.concat([result_df,new_df], ignore_index= True)
        except Exception as e:
            #handle the exception
            new_row = {'object_name':i,'query': query, 'Exception':str(e)}
            new_df = pd.DataFrame([new_row])
